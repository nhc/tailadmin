...
export const useTaskStateMachine = (
  ...
  const stateMachine = useMemo(
    () => createTaskStateMachine(taskStatus),
    [taskStatus]
  );

  const stateConfig = useMemo(
    () => getTaskStateConfig(taskStatus),
    [taskStatus]
  );
  ...
  return {
    ...
    canTransitionTo: (newState: TaskStatus) =>
      stateMachine.canTransitionTo(newState, userRole),
    isAssigned: stateMachine.isAssigned(),
